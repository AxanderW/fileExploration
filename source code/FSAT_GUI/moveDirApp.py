...
class MoveDirectoryClass(Ui_SpecialProjectsSecWindow ):
    def __init__(self):
    ...
    def setupUi(self, FMW):
        ...
        self.label.setText("Special Projects: Move Directory")
    ...
    def executeSlot(self):
        ...
        if rootdir != '' and targetfolder != '':
            try:
                self.debugPrint("Moving directory: {} to Target location: {}".format(rootdir,targetfolder))
                self.file_model.moveDirectory(self,rootdir,targetfolder)
                self.debugPrint("Directory: {} successfully moved to Target location: {}".format(rootdir,targetfolder))
            except:
